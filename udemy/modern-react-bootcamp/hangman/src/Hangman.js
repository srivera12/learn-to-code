...
class Hangman extends Component {
  ...
  */
  ...
  render() {
    ...
    if (gameWon) {
      gameState = `Congrats, you won! You guessed the word ${this.state.answer} with only ${this.state.nWrong} wrong guesses!`;
    } else if (gameOver) {
      gameState = `You lose. The secret word was: ${this.state.answer}`;
    }
    return (
      <div className="Hangman">
        <h1>Hangman</h1>
        <img src={this.props.images[this.state.nWrong]} alt={`${this.state.nWrong} wrong guesses`} />
        <p>You have made {this.state.nWrong} wrong guesses.</p>
